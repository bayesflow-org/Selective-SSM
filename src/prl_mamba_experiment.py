...
if __name__ == "__main__":
    ...
    experiment_params = budget_params
    ...
    for params in experiment_params:
        ...
        simulator = PRLSimulator(n_trials=100000, n_actions=3) # vary these?

        # Summary and inference networks
        summary_net = MambaSSM(summary_dim=16)
        # inference_net = bf.networks.CouplingFlow(coupling_kwargs={"subnet_kwargs": {"dropout": dropouts[budget]}})
        inference_net = bf.networks.CouplingFlow(coupling_kwargs={"subnet_kwargs": {"dropout": 0.3}})
        ...
        workflow = bf.BasicWorkflow(
            ...
            checkpoint_filepath="prl_mamba_results/checkpoints/" + create_fname(params),
        ...
        history = workflow.fit_offline(training_set, epochs=50, batch_size=64, validation_data=validation_set)
        ...
        with open("prl_mamba_results/diagnostics/" + create_fname(params, "latex") + ".tex", "w") as f:
            f.write(latex_df)
        
        # Save numeric diagnostics as csv
        df.to_csv("prl_mamba_results/diagnostics/" + create_fname(params, "table") + ".csv", header=True, index=True, float_format='%.3f')

        # Save figures
        for k, fig in figures.items():
            fig.savefig("prl_mamba_results/diagnostics/" + create_fname(params, k) + ".png", dpi=300)
