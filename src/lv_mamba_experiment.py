...
if __name__ == "__main__":
    ...
    for params in experiment_params:
        ...
        workflow = bf.BasicWorkflow(
            ...
            checkpoint_filepath="mamba_results/checkpoints/" + create_fname(params),
        ...
        with open("mamba_results/diagnostics/" + create_fname(params, "latex") + ".tex", "w") as f:
            f.write(latex_df)
        
        # Save numeric diagnostics as csv
        df.to_csv("mamba_results/diagnostics/" + create_fname(params, "table") + ".csv", header=True, index=True, float_format='%.3f')

        # Save figures
        for k, fig in figures.items():
            fig.savefig("mamba_results/diagnostics/" + create_fname(params, k) + ".png", dpi=300)
